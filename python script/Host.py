...
''' Bootloader Commands '''
CBL_GET_HELP_CMD				= 0x00
CBL_GET_VERSION_CMD 			= 0x01
CBL_GET_ID_CMD       			= 0x02
CBL_READ_MEMORY_CMD 			= 0x11
CBL_GO_TO_ADDR_CMD  			= 0x21
CBL_WRITE_MEMORY_CMD   			= 0x31
CBL_ERASE_CMD  				    = 0x43
CBL_EXTENDED_ERASE_CMD		    = 0x44
CBL_SPECIAL_CMD     			= 0x50
CBL_EXTENDED_SPECIAL_CMD   		= 0x51
CBL_WRITE_PROTECT_CMD		    = 0x63
CBL_WRITE_UNPROTECT_CMD 		= 0x73
CBL_READOUT_PROTECT_CMD	    	= 0x82
CBL_READOUT_UNPROTECT_CMD		= 0x92
CBL_CHECK_SUM_CMD			    = 0xA1
...
def Write_Data_To_Serial_Port(Value, Length):
    ...
    if verbose_mode:
        Value_bytes = bytearray(_data)
        print("   "+"0x{:02x}".format(Value_bytes[0]), end=' ')
...
def Read_Data_From_Serial_Port(Command_Code):
    ...
    if(len(BL_ACK)):
        ...
        if(BL_ACK_Array[0] == 0x79):
            ...
            elif (Command_Code == CBL_READ_MEMORY_CMD):
                Process_CBL_READ_MEMORY_CMD(Length_To_Follow)
            elif (Command_Code == CBL_ERASE_CMD):
                Process_CBL_FLASH_ERASE_CMD(Length_To_Follow)
            elif (Command_Code == CBL_WRITE_MEMORY_CMD):
                Process_CBL_MEM_WRITE_CMD(Length_To_Follow)
            elif (Command_Code == CBL_WRITE_UNPROTECT_CMD):
                Process_CBL_GET_WRP_STATUS_CMD(Length_To_Follow)
            elif (Command_Code == CBL_WRITE_PROTECT_CMD):
                Process_CBL_GET_WRP_STATUS_CMD(Length_To_Follow)    
        else:
            print ("\n   Received Not-Acknowledgement from Bootloader")
            sys.exit()

def Process_CBL_READ_MEMORY_CMD(Data_Len):
    global Memory_Write_Active
...
def Process_CBL_GET_WRP_STATUS_CMD(Data_Len):
    Serial_Data = Read_Serial_Port(Data_Len)
    _value_ = bytearray(Serial_Data)
    if(_value_[0] == 0xEE):
        print("\n   Write Protection State unknown !!")
    elif(_value_[0] == 0x00):
        print("\n   FLASH Write Protection : Disable")
    elif(_value_[0] == 0x01):
        print("\n   FLASH Write Protection : Enable")
...
def Decode_CBL_Command(Command):
    ...
    if (Command == 1):
        ...
        BL_Host_Buffer[1] = CBL_GET_VERSION_CMD
        ...
        Read_Data_From_Serial_Port(CBL_GET_ID_CMD)
    elif (Command == 4):
        print("Read Data From Memory") 
        CBL_READ_MEMORY_CMD_LEN = 12
        CBL_READ_Address = input("\n   Please Enter the Address in Hex : ")
        CBL_READ_Address = int(CBL_READ_Address, 16)
        CBL_NO_OF_BYTES_TO_READ = input("\n   Please Enter the Number of bytes to Read in Hex (ff Max): ")
        CBL_NO_OF_BYTES_TO_READ = int(CBL_NO_OF_BYTES_TO_READ, 16)
        '''CBL_NO_OF_BYTES_TO_READ = input("\n   Please Enter the Number of bytes in Hex (FF Max.NO): ")'''
        if not (0 < CBL_NO_OF_BYTES_TO_READ <= 255):
            print("\n Try command Again Enter the Number of bytes(255 Max.NO): ")
        else: 
            BL_Host_Buffer[0] = CBL_READ_MEMORY_CMD_LEN - 1
            BL_Host_Buffer[1] = CBL_READ_MEMORY_CMD
            BL_Host_Buffer[2] = Word_Value_To_Byte_Value(CBL_READ_Address, 1, 1) 
            BL_Host_Buffer[3] = Word_Value_To_Byte_Value(CBL_READ_Address, 2, 1) 
            BL_Host_Buffer[4] = Word_Value_To_Byte_Value(CBL_READ_Address, 3, 1) 
            BL_Host_Buffer[5] = Word_Value_To_Byte_Value(CBL_READ_Address, 4, 1)
            CRC32_Value = Calculate_CRC32(BL_Host_Buffer, CBL_READ_MEMORY_CMD_LEN - 6) 
            CRC32_Value = CRC32_Value & 0xFFFFFFFF
            ...
            BL_Host_Buffer[10] = CBL_NO_OF_BYTES_TO_READ - 1
            BL_Host_Buffer[11] = 0xff ^ BL_Host_Buffer[10]
            Write_Data_To_Serial_Port(BL_Host_Buffer[0], 1)
            for Data in BL_Host_Buffer[1 : CBL_READ_MEMORY_CMD_LEN]:
                Write_Data_To_Serial_Port(Data, CBL_READ_MEMORY_CMD_LEN - 1)
            Read_Data_From_Serial_Port(CBL_READ_MEMORY_CMD)
    elif (Command == 5):
        ...
        while(BinFileRemainingBytes):
            ...
            BL_Host_Buffer[1] = CBL_WRITE_MEMORY_CMD
            ...
            BL_Return_Value = Read_Data_From_Serial_Port(CBL_WRITE_MEMORY_CMD)
            sleep(0.1)
        ...
        BL_Host_Buffer[1] = CBL_ERASE_CMD
        ...
        Read_Data_From_Serial_Port(CBL_ERASE_CMD)
    elif (Command == 8):
        print("Extended Mass erase or sector erase of the user flash command")
    elif (Command == 9):
        print("New Special command")
    elif (Command == 10):
        print("New Extended Special command")  
    elif (Command == 11): 
        print("Enable Write protection the user flash command")
        #CBL_WRITE_PROTECT_CMD_Len = 15
        SectorNumber = 0
        NumberOfSectors = 0
        NumberOfSectors = input("\n   Please enter number of sectors to erase (7 Max)           : ")
        NumberOfSectors = int(NumberOfSectors, 16)
        CBL_WRITE_PROTECT_CMD_Len = (7+NumberOfSectors)
        BL_Host_Buffer = bytearray(3 + NumberOfSectors+4)
        BL_Host_Buffer[0] = CBL_WRITE_PROTECT_CMD_Len - 1
        BL_Host_Buffer[1] = CBL_WRITE_PROTECT_CMD
        if 1 <= NumberOfSectors <= 7:
            BL_Host_Buffer[2] = NumberOfSectors
            print("Enter {} SectorNumber:".format(NumberOfSectors))
            index = 3
            for i in range(NumberOfSectors):
                SectorNumber = int(input("Enter sector {} code in hex format (e.g., 0x6): ".format(i + 1)), 16)
                BL_Host_Buffer[index] = SectorNumber
                index += 1
        CRC32_Value = Calculate_CRC32(BL_Host_Buffer[:3+NumberOfSectors], CBL_WRITE_PROTECT_CMD_Len - 4) 
        CRC32_Value = CRC32_Value & 0xFFFFFFFF
        BL_Host_Buffer[3+NumberOfSectors] = Word_Value_To_Byte_Value(CRC32_Value, 1, 1)
        BL_Host_Buffer[4+NumberOfSectors] = Word_Value_To_Byte_Value(CRC32_Value, 2, 1)
        BL_Host_Buffer[5+NumberOfSectors] = Word_Value_To_Byte_Value(CRC32_Value, 3, 1)
        BL_Host_Buffer[6+NumberOfSectors] = Word_Value_To_Byte_Value(CRC32_Value, 4, 1)
        Write_Data_To_Serial_Port(BL_Host_Buffer[0], 1)
        for Data in BL_Host_Buffer[1 : CBL_WRITE_PROTECT_CMD_Len]:
            Write_Data_To_Serial_Port(Data, CBL_WRITE_PROTECT_CMD_Len - 1)
        Read_Data_From_Serial_Port(CBL_WRITE_PROTECT_CMD)
        '''
        if 1 <= NumberOfSectors <= 7:
            BL_Host_Buffer[2] = NumberOfSectors
            print("Enter {} SectorNumber:".format(NumberOfSectors))
        ...
        print("Disable the FLASH Write Protection State")
        CBL_GET_WRP_STATUS_CMD_Len = 6
        BL_Host_Buffer[0] = CBL_GET_WRP_STATUS_CMD_Len - 1
        BL_Host_Buffer[1] = CBL_WRITE_UNPROTECT_CMD
        CRC32_Value = Calculate_CRC32(BL_Host_Buffer, CBL_GET_WRP_STATUS_CMD_Len - 4)
        CRC32_Value = CRC32_Value & 0xFFFFFFFF
        BL_Host_Buffer[2] = Word_Value_To_Byte_Value(CRC32_Value, 1, 1)
        BL_Host_Buffer[3] = Word_Value_To_Byte_Value(CRC32_Value, 2, 1)
        BL_Host_Buffer[4] = Word_Value_To_Byte_Value(CRC32_Value, 3, 1)
        BL_Host_Buffer[5] = Word_Value_To_Byte_Value(CRC32_Value, 4, 1)
        Write_Data_To_Serial_Port(BL_Host_Buffer[0], 1)
        for Data in BL_Host_Buffer[1 : CBL_GET_WRP_STATUS_CMD_Len]:
            Write_Data_To_Serial_Port(Data, CBL_GET_WRP_STATUS_CMD_Len - 1)
        Read_Data_From_Serial_Port(CBL_WRITE_UNPROTECT_CMD)            
    elif (Command == 13):
        ...
        if(Protection_level == 2):
            print("\n   Protection level (2) not supported to avoid lose MC at this time !!")
            ...
            BL_Host_Buffer[1] = CBL_READOUT_PROTECT_CMD
            ...
            Read_Data_From_Serial_Port(CBL_READOUT_PROTECT_CMD)
        else:
        ...
        BL_Host_Buffer[1] = CBL_READOUT_UNPROTECT_CMD
        ...
        Read_Data_From_Serial_Port(CBL_READOUT_UNPROTECT_CMD)        
...
while True:
    print("\nSTM32F756ZG Custome BootLoader")
    print("==============================")
    print("Which command you need to send to the bootLoader :")
    print("   CBL_GET_HELP_CMD                  --> 1")
    print("   CBL_GET_VERSION_CMD               --> 2")
    print("   CBL_GET_ID_CMD                    --> 3")
    print("   CBL_READ_MEMORY_CMD               --> 4")
    print("   CBL_GO_TO_ADDR_CMD                --> 5")
    print("   CBL_WRITE_MEMORY_CMD              --> 6")
    print("   CBL_ERASE_CMD                     --> 7")
    print("   CBL_EXTENDED_ERASE_CMD            --> 8")
    print("   CBL_SPECIAL_CMD                   --> 9")
    print("   CBL_EXTENDED_SPECIAL_CMD          --> 10")
    print("   CBL_WRITE_PROTECT_CMD             --> 11")
    print("   CBL_WRITE_UNPROTECT_CMD           --> 12")
    print("   CBL_READOUT_PROTECT_CMD           --> 13")
    print("   CBL_READOUT_UNPROTECT_CMD         --> 14")
    print("   CBL_CHECK_SUM_CMD                 --> 15")
